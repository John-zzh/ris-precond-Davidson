...
def gen_global_var(tddft, mf, mol, functional=args.functional):
    ...
    if no truncation, then trunced_vir = n_vir and n_occ + trunced_vir = N_bf
    '''
    ...
    N_bf:
        ...
        truncated_occ =             n_occ - truncated_occ      trunced_vir         n_vir - trunced_vir
    ...
    if args.truncate_virtual> 1:
        tol_eV = args.truncate_virtual/parameter.Hartree_to_eV
        homo_vir = delta_hdiag[-1,:]
        trunced_vir = len(np.where(homo_vir <= tol_eV)[0])
    else:
        ''' the percengate of truncation,
            e.g.0.3 means for 30% virtual orbitals are throw away
        '''
        trunced_vir = int(n_vir*(1-args.truncate_virtual))

    if args.truncate_occupied > 0:
        truncated_occ = int(n_occ*args.truncate_occupied)
    ...
    delta_hdiag:
                    --------------|-----------------------
    truncated_occ   |    hdiag1   |                      |
                    --------------|                      |
    n_occ -         |             |        hdiag3        |
    truncated_occ   |    hdiag2   |                      |
                    |             |                      |
                    --------------|-----------------------
                         trunced_vir        n_vir - trunced_vir
    '''
    ...
    delta_hdiag2 = delta_hdiag[:,:trunced_vir]
    delta_hdiag3 = delta_hdiag[:,trunced_vir:]

    A_reduced_size = n_occ * trunced_vir
    ...
    print('n_occ = ', n_occ)
    print('n_vir = ', n_vir)
    print('trunced_vir = ', trunced_vir)
    print('A_size = ', A_size)
    print('A_reduced_size =', A_reduced_size)

    return (TDA_vind, TDDFT_vind, hdiag, delta_hdiag, delta_hdiag2,
        delta_hdiag3, N_atm, un_ortho_C_matrix, C_matrix, N_bf, n_occ, n_vir,
        trunced_vir, A_size, A_reduced_size, R_array, a_x, beta, alpha)

(TDA_vind, TDDFT_vind, hdiag, delta_hdiag, delta_hdiag2, delta_hdiag3, N_atm,
un_ortho_C_matrix, C_matrix, N_bf, n_occ, n_vir, trunced_vir, A_size,
...
def delta_hdiag3_fly(V):
    '''
    delta_hdiag2.shape = (n_occ, trunced_vir)
    '''
    delta_trunced_vir_v = einsum("ia,iam->iam", delta_hdiag2, V)
    return delta_trunced_vir_v

def delta_hdiag3_fly(V):
    '''
    delta_hdiag2.shape = (n_occ, n_vir-trunced_vir)
    '''
    delta_rst_vir_v = einsum("ia,iam->iam", delta_hdiag3, V)
...
TDDFT_vind, hdiag, delta_hdiag, delta_hdiag2, delta_hdiag3, N_atm,
un_ortho_C_matrix, C_matrix, N_bf, n_occ, n_vir, trunced_vir, A_size,
