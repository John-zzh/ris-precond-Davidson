...
from SCF_calc import (alpha, beta, mol, N_atm, N_bf, n_occ, n_vir, max_vir,
                    R_array, C_matrix, eta, a_x, delta_hdiag, max_vir_hdiag,
                    rst_vir_hdiag)
...
class sTDA(object):
    ...
    def gen_mv_fly(self, iajb_fly, ijab_fly, ibja_fly, delta_fly,
    ...
    def build(self, eta=eta):

        (GammaJ,
        GammaK) = self.gen_gammaJK(eta)
        ...
        delta_fly,
        delta_max_vir_fly,
        delta_rst_vir_fly) = self.gen_iajb_ijab_ibja_delta_fly(
                            q_ij=q_ij,
                            q_ab=q_ab,
                            q_ia=q_ia,
                            GK_q_jb=GK_q_jb,
        ...
        self.full_sTDA_mv,
        self.sTDDFT_mv,
        self.sTDDFT_stapol_mv) = self.gen_mv_fly(
                                iajb_fly=iajb_fly,
                                ijab_fly=ijab_fly,
                                ibja_fly=ibja_fly,
                                delta_fly=delta_fly,
                                delta_max_vir_fly=delta_max_vir_fly,
                                delta_rst_vir_fly=delta_rst_vir_fly)

sTDA = sTDA()
sTDA.build()

sTDA_mv = sTDA.sTDA_mv
full_sTDA_mv = sTDA.full_sTDA_mv
sTDDFT_mv = sTDA.full_sTDA_mv
sTDDFT_stapol_mv = sTDA.sTDDFT_stapol_mv
