...
def gen_args():
    ...
    parser.add_argument('-TDDFTas','--TDDFTas',     type=str2bool,  default = False, help='perform TDDFTas calculation')
...
GAMMA_ij, GAMMA_ab, GAMMA_ia, GAMMA_J_ia, GAMMA_J_ij = gen_GAMMA()
...
def sTDA_eigen_solver(k, tol=args.initial_TOL, matrix_vector_product=sTDA_mv):
    ...
    V = np.zeros((A_reduced_size, (max+1)*k + m))
    W = np.zeros_like(V)
...
def gen_forecaster(eta, *args):
    ...
    GammaJ, GammaK = gen_gammaJK(alpha=alpha, beta=beta, eta=eta)
